...
public class InlineTest {
  ...
  /** Tests a {@code case} applied to a constant is inlined. */
  @Test
  void testInlineCase3() {
    final String ml =
        "let\n"
            + "  val x = 2\n"
            + "in\n"
            + "  case x of 1 => \"one\" | 2 => \"two\" | _ => \"large\"\n"
            + "end";
    ml(ml)
        .assertCore(
            0,
            hasToString(
                "val it = "
                    + "let val x = 2 "
                    + "in case x of 1 => \"one\" | 2 => \"two\" | _ => \"large\" "
                    + "end"))
        .assertCore(2, hasToString("val it = \"two\""))
        .assertEval(is("two"));
  }

  /** Tests a {@code case} matching the first literal pattern. */
  @Test
  void testInlineCase4() {
    final String ml =
        "let\n"
            + "  val x = 1\n"
            + "in\n"
            + "  case x of 1 => \"one\" | 2 => \"two\" | _ => \"large\"\n"
            + "end";
    ml(ml).assertCore(2, hasToString("val it = \"one\"")).assertEval(is("one"));
  }

  /** Tests a {@code case} matching the wildcard pattern. */
  @Test
  void testInlineCase5() {
    final String ml =
        "let\n"
            + "  val x = 99\n"
            + "in\n"
            ...
            + "end";
    ml(ml)
        .assertCore(2, hasToString("val it = \"large\""))
        .assertEval(is("large"));
  }

  /**
   * Tests a {@code case} with an identifier pattern that binds the constant.
   */
  @Test
  void testInlineCase6() {
    final String ml =
        "let\n"
            + "  val x = 5\n"
            + "in\n"
            ...
            + "end";
    ml(ml)
        .assertCore(2, hasToString("val it = let val y = 5 in y + 10 end"))
        .assertCore(-1, hasToString("val it = 5 + 10"))
        .assertEval(is(15));
  }
}
