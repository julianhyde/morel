 ...
 *
 * <p>The algorithm converts a list of patterns into a boolean formula with
 * several variables, then checks whether the formula is satisfiable (that is,
 * whether there is a combination of assignments of boolean values to the
...
class PatternCoverageChecker {
   ...
   * {@code pat} would already have been matched by one or more of {@code
   * prevPatList}.
   *
   * <p>For example, the pattern "(1, b: bool)" is covered by "[(1, true), (_,
   * false)]" but not by "[(1, true)]" or "[(_, false)]".
   */
  static boolean isCoveredBy(
      TypeSystem typeSystem, List<Core.Pat> prevPatList, Core.Pat pat) {
  ...
  /**
   * Returns whether a list of patterns covers every possible value. If so, any
   * pattern added to this list would be redundant.
   */
  ...
  static boolean isExhaustive(TypeSystem typeSystem, List<Core.Pat> patList) {
    ...
    if (Iterables.any(
        patList, p -> p.op == Op.WILDCARD_PAT || p.op == Op.ID_PAT)) {
      return true; // shortcut
    }
    final Core.WildcardPat wildcardPat = core.wildcardPat(patList.get(0).type);
  ...
  private void toTerm(Core.Pat pat, Path path, List<Sat.Term> terms) {
    switch (pat.op) {
      ...
      case TUPLE_PAT:
        ...
        forEachIndexed(
            tuplePat.args, (pat2, i) -> toTerm(pat2, path.sub(i), terms));
        return;

      case RECORD_PAT:
        final Core.RecordPat recordPat = (Core.RecordPat) pat;
        forEachIndexed(
            recordPat.args, (pat2, i) -> toTerm(pat2, path.sub(i), terms));
        return;
   ...
   *
   * <p>For example, converts: "[]" to "NIL", "[x]" to "CONS (x, NIL)", "[x, y]"
   * to "CONS (x, CONS (y, NIL))", etc.
   */
  private Core.Pat listToCons(Core.ListPat listPat) {
    final Type listType = typeSystem.lookup(BuiltIn.Datatype.PSEUDO_LIST);
    ...
    if (args.isEmpty()) {
      ...
      return core.consPat(
          ...
          core.tuplePat(
              typeSystem,
              ImmutableList.of(
                  args.get(0), listToConsRecurse(listDataType, skip(args)))));
    }
  }

  private void addConsTerms(
      Path path, List<Sat.Term> terms, Core.TuplePat tuplePat) {
   ...
   * the patterns in {@code patList} before reaching {@code pat}. Thus {@code
   * pat} is said to be <dfn>redundant</dfn> in that context, and could be
   ...
   *
   * <p>Paths are basically immutable lists of integers, built by appending one
   * element at a time.
   */
  private abstract static class Path {
     ...
     *
     * <p>{@code ROOT.sub(2).sub(1).toVar("x")} will return "2.1.x".
     */
  ...
  /**
   * Path that is a child of a given parent path. The {@code ordinal} makes it
   * unique within its parent. For tuple and record patterns, {@code ordinal} is
   * the field ordinal.
   */
  ...
  /**
   * Payload of a {@code Sat.Variable} that is an algebraic type. There are
   * sub-variables representing whether the tag holds each of its allowed values
   * (each of which is a constructor).
   */
