...
public class Tracers {
  ...
  private static class ConfigurableTracerImpl implements ConfigurableTracer {
    ...
    @Override public ConfigurableTracer withDeleteHandler(
        BiConsumer<Term, Term> deleteHandler) {
      return new ConfigurableTracerImpl(deleteHandler, conflictHandler,
          sequenceHandler, swapHandler, cycleHandler, variableHandler,
          substituteHandler);
    }

    @Override public ConfigurableTracer withConflictHandler(
        BiConsumer<Sequence, Sequence> conflictHandler) {
      return new ConfigurableTracerImpl(deleteHandler, conflictHandler,
          sequenceHandler, swapHandler, cycleHandler, variableHandler,
          substituteHandler);
    ...
    @Override public ConfigurableTracer withSequenceHandler(
        BiConsumer<Sequence, Sequence> sequenceHandler) {
      return new ConfigurableTracerImpl(deleteHandler, conflictHandler,
          sequenceHandler, swapHandler, cycleHandler, variableHandler,
          substituteHandler);
    ...
    @Override public ConfigurableTracer withSwapHandler(
        BiConsumer<Term, Term> swapHandler) {
      return new ConfigurableTracerImpl(deleteHandler, conflictHandler,
          sequenceHandler, swapHandler, cycleHandler, variableHandler,
          substituteHandler);
    ...
    @Override public ConfigurableTracer withCycleHandler(
        BiConsumer<Variable, Term> cycleHandler) {
      return new ConfigurableTracerImpl(deleteHandler, conflictHandler,
          sequenceHandler, swapHandler, cycleHandler, variableHandler,
          substituteHandler);
    ...
    @Override public ConfigurableTracer withVariableHandler(
        BiConsumer<Variable, Term> variableHandler) {
      return new ConfigurableTracerImpl(deleteHandler, conflictHandler,
          sequenceHandler, swapHandler, cycleHandler, variableHandler,
          substituteHandler);
    ...
    @Override public ConfigurableTracer withSubstituteHandler(
        QuadConsumer<Term, Term, Term, Term> substituteHandler) {
      return new ConfigurableTracerImpl(deleteHandler, conflictHandler,
          sequenceHandler, swapHandler, cycleHandler, variableHandler,
          substituteHandler);
    ...
    @Override public void onDelete(Term left, Term right) {
      deleteHandler.accept(left, right);
    }

    @Override public void onConflict(Sequence left, Sequence right) {
      conflictHandler.accept(left, right);
    }

    @Override public void onSequence(Sequence left, Sequence right) {
      sequenceHandler.accept(left, right);
    }

    @Override public void onSwap(Term left, Term right) {
      swapHandler.accept(left, right);
    }

    @Override public void onCycle(Variable variable, Term term) {
      cycleHandler.accept(variable, term);
    }

    @Override public void onVariable(Variable variable, Term term) {
      variableHandler.accept(variable, term);
    }

    @Override public void onSubstitute(Term left, Term right, Term left2,
        Term right2) {
