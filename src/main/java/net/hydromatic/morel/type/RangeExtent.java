...
public class RangeExtent {
   ...
   * </ul>
   *
   * <p>Using a map {@code ["/O/SOME/" "[0, 3], [6]"]} we can generate {@code
   * SOME 0, SOME 1, SOME 2, SOME 3, SOME 6, NONE}
   */
   ...
   * its type.
   *
   * <p>Examples: "(-inf,+inf)" (true), "(-inf,0]" (x &le; 0),
   * "{(-inf,3),(10,+inf)}" (x &lt; 3 or x &gt; 10) are unbounded; "{}" (false),
   * "{3, 10}" (x in [3, 10]), "(3, 10)" (x &ge; 3 andalso x &le; 10) are
   * bounded.
   */
   ...
   */
  @SuppressWarnings("unchecked")
  private <E extends Comparable<E>> boolean populate(
      TypeSystem typeSystem,
      Type type,
      String path,
      Map<String, ImmutableRangeSet> rangeSetMap,
    ...
    switch (type.op()) {
      ...
      case DATA_TYPE:
        ...
        for (Map.Entry<String, Type> entry :
          ...
          if (!populate(
              typeSystem, type2, path + name + "/", rangeSetMap, consumer2)) {
            return false;
          }
        }
        return true;
      ...
      case TUPLE_TYPE:
        ...
        Lists.cartesianProduct(listList)
            .forEach(
                list ->
                    filteredConsumer.accept(
                        (E) FlatLists.ofComparable((List) list)));
        return true;
