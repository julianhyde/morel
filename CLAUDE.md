...
- In expected output, a top-level string value that contains a newline (and no
  space or tab before a newline) is written as a raw string literal, `{|...|}`
  (or `{id|...|id}` if the content contains `|}`, the tag being lower-case
  letters and underscores), with verbatim content; the harness generates this
  form, and `OutputMatcher` treats it as equivalent to the escaped literal.
