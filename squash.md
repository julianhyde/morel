...
2. **M3 will not collapse to empty**, and Phase 3 settled why: it is the
   branch's alias work, not `d1a013a4`, because `d1a013a4` does not apply to
   `d4eeff5a`. Expect a residue and read it: it is the difference between what
   the branch had and what was squashed into #459, and the known part of it is
   `Unifier.weaken`, which the branch moved out of `MartelliUnifier` for
   `eraseConstraints` to use. Fold what is real into M4 and drop the rest.
