...
| output               | enum   | doc     | How values should be formatted. "doc" (the default) lays values out with a Doc-based pretty-printer that follows SML/NJ's line breaking; "classic" prints values in a compact nested format; "tabular" prints values in a table if their type is a list of records. |
