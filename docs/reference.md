...
| Value.parse | string &rarr; value | "parse s" parses a value from its string representation.  The string is in the format produced by the `print` function, and therefore `parse (print v) = v` for all values `v`. |
| Value.print | value &rarr; string | "print v" converts a value to a string.  For example, `print (BOOL true)` returns `"BOOL true"`; `print (LIST [INT 1, INT 2])` returns `"LIST [INT 1, INT 2]"`. |
