...
| Relational.iterate, iterate | &alpha; bag &rarr; (&alpha; bag * &alpha; bag &rarr; &alpha; bag) &rarr; &alpha; bag | "iterate initialList listUpdate" computes a fixed point, starting with `initialList` and calling `listUpdate prevList newList` each iteration, terminating the iteration when it returns `newList`. |
...
| Vector.collate | (&alpha; * &alpha; -&gt; order) -&gt; &alpha; vector * &alpha; vector -&gt; order | "collate f (v1, v2)" performs lexicographic comparison of the two vectors using the given ordering `f` on elements. |
...
| Vector.exists | (&alpha; -&gt; bool) -&gt; &alpha; vector -&gt; bool | "exists f vec" applies `f` to each element `x` of the vector `vec`, from left to right (i.e., increasing indices), until `f(x)` evaluates to `true`; it returns `true` if such an `x` exists and `false` otherwise. |
