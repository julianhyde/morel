...
| output               | enum   | classic | How values should be formatted. "classic" (the default) prints values in a compact nested format; "tabular" prints values in a table if their type is a list of records; "doc" is like "classic" but lays values out with a Doc-based pretty-printer (experimental). |
